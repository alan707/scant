...
10. **Never hardcode what the environment can tell you.** No baked-in lists of package names, driver mappings, framework conventions, or import-name translations — and don't propose one as the fix. The fact is almost always already on disk: `METADATA`, `RECORD`, `entry_points.txt`, or the installed package's own source. Reading it is correct today *and* after the next release. A list is a snapshot of one person's memory: it goes stale in silence, it can't explain itself in the `WHERE` column, and it only ever covers what its author happened to think of. Decided against a proposed override table of database drivers — reading installed source instead surfaced `azure-storage-common` imported by `celery/backends/azureblockblob.py:16`, a relationship no table anyone wrote would have contained. When nothing on disk records the fact, that is a signal the feature needs a different mechanism, not a list. If a list is genuinely unavoidable, it is opt-in, small, and every entry cites a named specimen from a real repo. (#7's version-keyed stdlib sets are the standing exception, and even those should be asked of the target interpreter — `sys.stdlib_module_names` — wherever it can be asked.)
