...
### Added
- Installed commands are now read from `RECORD`, so a dependency that ships a prebuilt executable rather than declaring a `console_scripts` entry point is recognized as `registered` instead of `drop`. `entry_points.txt` records what a package *declares*; `RECORD` records what actually got *installed*, and maturin-built tools (`ruff`, `uv`) ship a binary with no `entry_points.txt` at all. The signal is a `RECORD` path escaping site-packages into `bin/` or `Scripts/` (e.g. `../../../bin/ruff`), matched on the directory rather than a fixed depth, since that varies by layout; identical under `pip` and `uv`. These paths were previously skipped outright, which was correct for collecting import roots -- they still never become one -- but discarded the only evidence such tools leave behind.
